...
class mongodb inherits mongodb::params {
	...
	define mongod (
		...
		file {
			"/etc/mongodb/${mongod_instance}.conf":
				content => template('mongodb/mongod.conf.erb'),
				mode    => '0644',
				# no auto restart of a db because of a config change
				#	notify  => Class['mongodb::service'],
				require => [Class['mongodb::install'],File['/etc/mongodb']];
			"/etc/mongodb":
				ensure  => 'directory',
				mode    => '0755',
				# no auto restart of a db because of a config change
			...
			"/etc/init.d/mongod_${mongod_instance}":
				...
				mode    => '0755',
				require => Class['mongodb::install'];
			"${::mongodb::params::dbdir}/${mongod_instance}":
				ensure  => 'directory',
				mode    => '0755',
				owner    => "${::mongodb::params::run_as_user}",
				group   => "${::mongodb::params::run_as_group}",
				before  => Service["mongod_${mongod_instance}"],
				require => [Class['mongodb::install'], File["${::mongodb::params::dbdir}"]];
			"${::mongodb::params::dbdir}/${mongod_instance}/mongod.lock":
				ensure  => 'absent',
				before  => Service["mongod_${mongod_instance}"],
				require => [
					Class['mongodb::install'],
					File["${::mongodb::params::dbdir}/${mongod_instance}"]
				];
			"${::mongodb::params::dbdir}":
				ensure  => 'directory',
				mode    => '0755',
				owner    => "${::mongodb::params::run_as_user}",
				group   => "${::mongodb::params::run_as_group}",
